...
class ModeOrchestrator extends ValueNotifier<ModeState> {
  ...
  static Map<JanarymMode, ModeDescriptor> _buildDescriptors() {
    return <JanarymMode, ModeDescriptor>{
      ...
      JanarymMode.bus: ModeDescriptor(
        mode: JanarymMode.bus,
        contextKey: 'bus',
        ui: const ModeUiIndicator(
          labelRu: 'Автобусы',
          labelKk: 'Автобустар',
        ...
        perception: const ModePerceptionFilter(
          requiresLiveCamera: false,
          prefersNavigationGuidance: false,
          showHazardOverlay: false,
          allowHazardVoice: false,
        ),
        prompts: const ModePromptProfile(
          blindRu:
              'Это bus режим. Отвечай только про автобусы: какая остановка ближе, какие маршруты там ходят и когда придет нужный автобус.',
          blindKk:
              'Бұл bus режимі. Тек автобустар бойынша жауап бер: қай аялдама жақын, онда қандай маршруттар бар және керек автобус қашан келеді.',
          visionRu:
              'Это bus режим. Камера тут не приоритет. Если пользователь просит про автобус, отвечай по остановкам и расписанию.',
          visionKk:
              'Бұл bus режимі. Камера мұнда басым емес. Пайдаланушы автобус туралы сұраса, аялдама мен кесте бойынша жауап бер.',
        ),
      ),
      ...
      JanarymMode.anti_fraud: ModeDescriptor(
        ...
        prompts: const ModePromptProfile(
          blindRu:
              'Это anti_fraud режим. Сначала ищи в OCR явные маркеры фальшивки или сувенира: "не является платежным средством", "сувенир" и похожие надписи. Если маркеры есть, скажи прямо, что это сувенир или подделка. Если уверенности недостаточно, честно скажи, что не можешь подтвердить подлинность. Назови номинал, только если он различим.',
          blindKk:
              'Бұл anti_fraud режимі. Алдымен OCR арқылы жалған немесе кәдесый белгілерін ізде: "төлем құралы болып табылмайды", "сувенир" және ұқсас жазулар. Егер ондай белгі болса, бұл кәдесый не жалған ақша екенін тікелей айт. Егер сенім жеткіліксіз болса, түпнұсқалығын растай алмайтыныңды ашық айт. Номиналды тек анық көрінсе ғана ата.',
          visionRu:
              'Это anti_fraud режим. Проверь купюру (тенге) по фото и OCR. Ищи явные сувенирные надписи и несоответствия. Если на фото недостаточно данных, не угадывай: скажи, что не можешь подтвердить подлинность. Номинал называй только если он различим.',
          visionKk:
              'Бұл anti_fraud режимі. Купюраны (теңге) фото мен OCR бойынша тексер. Кәдесый жазуларын және айқын сәйкессіздіктерді ізде. Егер дерек жеткіліксіз болса, болжай салма: түпнұсқалығын растай алмайтыныңды айт. Номиналды тек анық көрінсе ғана ата.',
        ),
      ),
      JanarymMode.memory: ModeDescriptor(
        ...
        perception: const ModePerceptionFilter(enableSceneMemory: false),
        prompts: const ModePromptProfile(
          blindRu:
              'Это memory режим. Сохраняй и вспоминай только текстовые заметки, коды и короткие якоря по имени. Не обещай сохранение кадров, фото или сцены.',
          blindKk:
              'Бұл memory режимі. Тек мәтіндік жазбаларды, кодтарды және қысқа якорьлерді атауы бойынша сақта және еске түсір. Кадрды, фотоны не сахнаны сақтаймын деп уәде берме.',
          visionRu:
              'Это memory режим. Даже если есть изображение, не обещай сохранить кадр или сцену. Проси продиктовать текст заметки или код.',
          visionKk:
              'Бұл memory режимі. Сурет болса да кадрды не сахнаны сақтаймын деп айтпа. Пайдаланушыдан жазбаның мәтінін не кодын айтуын сұра.',
        ),
      ),
